...
import (
	...
	zrpclog "github.com/rooobot/zrpc/log"
)
...
func (g *Generator) CommandLineParameters(parameter string) {
	...
	zrpclog.Debugf("pluginlist : %v", pluginList)
	if pluginList != "" {
		...
		for _, p := range plugins {
			zrpclog.Debugf("enabled : %v", enabled)
			zrpclog.Debugf("p.Name() : %v", p.Name())
			if enabled[p.Name()] {
				nplugins = append(nplugins, p)
			}
		}
		zrpclog.Debugf("nplugins : %v", nplugins)
		plugins = nplugins
	}
}
...
func (g *Generator) runPlugins(file *FileDescriptor) {
	zrpclog.Debugf("plugins : %v", plugins)
	zrpclog.Debugf("len(plugins) : %d", len(plugins))
	for _, p := range plugins {
		p.Generate(file)
	}
}
